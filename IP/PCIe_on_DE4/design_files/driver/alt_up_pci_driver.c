...
static ssize_t alt_up_pci_write  (struct file *filp, const char __user *buf, size_t count, loff_t *f_pos) {
	...
	// error checking
	if( *f_pos + count > mydev->bar_size[mydev->rw_bar_no] ){
	...
	if( (count % 4 == 0) && (*f_pos % 4 == 0) ){
		while( count > 0 ){
			...
			iowrite32( ((u32 *)kernel_buf)[bytes_written/4], mydev->bar[mydev->rw_bar_no] + *f_pos ); 
		...
		while( count > 0 ){
			...
			iowrite8( kernel_buf[bytes_written], mydev->bar[mydev->rw_bar_no] + *f_pos ); 
...
static ssize_t alt_up_pci_read   (struct file *filp, char __user *buf, size_t count, loff_t *f_pos){
	...
	if ( (count % 4 == 0) && (*f_pos % 4 == 0) ){
		while( count > 0 ) {
			...
			((u32 *)kernel_buf)[bytes_read/4] = ioread32(mydev->bar[mydev->rw_bar_no] + *f_pos); 
		...
		while( count > 0 ) {
			...
			kernel_buf[bytes_read] = ioread8(mydev->bar[mydev->rw_bar_no] + *f_pos); 
...
static long alt_up_pci_ioctl_unlocked(struct file *filp, unsigned int cmd, unsigned long arg) {
	...
	switch(cmd){  
		...
		case ALT_UP_IOCTL_DMA_ADD: // add a new DMA transaction into the queue of the selected DMA controller
			...
			// check the rooms in the queue	
			if( myctrller->queue_end == DMA_MAX_QUEUE_NUM ){
				printk(KERN_DEBUG "The queue is full. \n");
				mutex_unlock (&alt_up_pci_lock);
				return -EFAULT;
			}
		
			// set the queue entry
			myctrller->dma_queue[myctrller->queue_end].user_buffer_addr  = handler.user_buffer_addr;
			myctrller->dma_queue[myctrller->queue_end].dma_rw_slave_addr = handler.dma_rw_slave_addr;
			myctrller->dma_queue[myctrller->queue_end].length            = handler.dma_length_byte;
			myctrller->dma_queue[myctrller->queue_end].to_device         = handler.to_device;
		...
		case ALT_UP_IOCTL_DMA_GO: // set GO for DMA transfer in the queue
			...
			if( handler.dma_controller_id < 0 || handler.dma_controller_id >= MAX_NUM_OF_DMAS ) {
				...
				mutex_unlock (&alt_up_pci_lock);
				return -EFAULT;
			} else if( handler.use_interrupt != 0 && handler.use_interrupt != 1){

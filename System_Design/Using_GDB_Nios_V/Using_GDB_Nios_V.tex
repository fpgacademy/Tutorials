...
A GDB command reference is provided in Appendix A. Of course, a lot of documentation about
GDB commands can also be found on the Internet. 
...
which corresponds to the label \texttt{loop}, by using the command \texttt{break} ({\it b}) 7.  
Then, run to this breakpoint twice by using the \texttt{continue} (\texttt{c})
...
\begin{figure}[H]
\lstinputlisting[language=C, lastline=71]{Code/handlers.c}
